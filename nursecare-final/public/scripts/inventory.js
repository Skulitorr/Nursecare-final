...
const DEFAULT_ITEMS = [
    {
        id: 'gloves',
        name: 'Hanskar',
        category: CATEGORIES.MEDICAL_SUPPLIES,
        stock: 12,
        unit: 'box',
        minStock: 20,
        maxStock: 100
...
function saveInventoryItems(items) {
    try {
        ...
        showToast('Villa', 'Villa kom upp við að vista birgðir', 'error');
    }
}
...
function createInventoryItemElement(item) {
    ...
    itemEl.innerHTML = `
        <div class="item-header">
            ...
            <span class="item-status" style="background-color: ${status.color}">
                ${status.label}
            </span>
        </div>
        <div class="item-details">
            ...
            <div class="stock-bar">
                <div class="stock-fill" style="width: ${(item.stock / item.maxStock) * 100}%"></div>
            </div>
        </div>
        ${hasActionAccess(getCurrentUser()?.role, 'manage_inventory') ? `
            <div class="item-actions">
                <button class="edit-item" aria-label="Breyta">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="adjust-stock" aria-label="Breyta magni">
 ...
 * @param {number} max - Maximum stock level
 * @returns {Object} Status object with color and label
 */
function getStockStatus(current, min, max) {
    const percentage = (current / max) * 100;
    
    if (percentage <= STATUS_LEVELS.CRITICAL.threshold) {
        return {
            color: STATUS_LEVELS.CRITICAL.color,
            label: STATUS_LEVELS.CRITICAL.label
        };
    } else if (percentage <= STATUS_LEVELS.LOW.threshold) {
        return {
            color: STATUS_LEVELS.LOW.color,
            label: STATUS_LEVELS.LOW.label
        };
    } else if (percentage <= STATUS_LEVELS.NORMAL.threshold) {
        return {
            color: STATUS_LEVELS.NORMAL.color,
            label: STATUS_LEVELS.NORMAL.label
        };
    } else {
        return {
            color: STATUS_LEVELS.EXCESS.color,
            label: STATUS_LEVELS.EXCESS.label
        };
    }
}
...
function setupInventoryFilters() {
    ...
    if (filterInput) {
        filterInput.addEventListener('input', filterInventoryItems);
    }
    ...
    if (categorySelect) {
        ...
        categorySelect.addEventListener('change', filterInventoryItems);
    }
    
    if (statusSelect) {
        // Populate status levels
        Object.entries(STATUS_LEVELS).forEach(([key, value]) => {
            const option = document.createElement('option');
            option.value = key;
        ...
        statusSelect.addEventListener('change', filterInventoryItems);
    }
}
...
function getItemStatus(itemEl) {
    ...
    const backgroundColor = getComputedStyle(statusEl).backgroundColor;
    
    // Convert background color to status key
    return Object.entries(STATUS_LEVELS).find(([_, value]) => {
        return rgbToHex(backgroundColor) === value.color;
    })?.[0] || '';
}
...
function setupEventListeners() {
    ...
    if (addItemBtn && hasActionAccess(getCurrentUser()?.role, 'manage_inventory')) {
    ...
    document.addEventListener('click', e => {
        ...
        const adjustBtn = e.target.closest('.adjust-stock');
        
        if (editBtn) {
            const itemEl = editBtn.closest('.inventory-item');
            const itemId = itemEl.getAttribute('data-id');
            showEditItemModal(itemId);
        }
        
        if (adjustBtn) {
            const itemEl = adjustBtn.closest('.inventory-item');
            const itemId = itemEl.getAttribute('data-id');
...
function showEditItemModal(itemId) {
    // Implementation would go here
    console.log('Show edit item modal', itemId);
}

/**
 * Show adjust stock modal
 * @param {string} itemId - Item ID to adjust
 */
function showAdjustStockModal(itemId) {
    // Implementation would go here
    console.log('Show adjust stock modal', itemId);
}
...
function updateInventoryStats() {
    ...
    Object.entries(stats).forEach(([key, value]) => {
        const el = document.querySelector(`.inventory-stat.${key} .stat-value`);
